...
pub async fn run(
    ...
    s3::download_file(s3_client, &config.s3_bucket, &original_key, &original).await?;
    ...
    match kind.as_str() {
        ...
        "image" => {
            ...
            s3::upload_file(s3_client, &config.s3_bucket, &key, &thumb_path, "image/jpeg")
                .await?;
...
async fn process_video(
    ...
    // Thumbnail at 1s or midpoint (whichever is earlier, for short clips)
    ...
    s3::upload_file(s3_client, &config.s3_bucket, &thumb_key, &thumb_path, "image/jpeg").await?;
    ...
    if probe.has_audio {
        ...
        s3::upload_file(s3_client, &config.s3_bucket, &wf_key, &waveform_path, "application/json")
            .await?;
