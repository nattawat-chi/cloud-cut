...
/**
 * Floating cursors for collaborators whose `target === 'viewport'`.
 *
 * Mounted inside the center panel so the (x, y) ratios resolve relative to
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-[999]">
      {Object.entries(cursors).map(([uid, cu]) => {
        if (cu.target !== 'viewport' || !cu.visible || cu.x == null || cu.y == null) return null;
        ...
        return (
          ...
          >
            ...
            <div
              className="absolute whitespace-nowrap rounded-r-md rounded-b-md px-1.5 py-px text-[10.5px] font-semibold text-white"
